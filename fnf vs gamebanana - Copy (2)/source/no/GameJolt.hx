...
class GameJoltLogin extends MusicBeatState
{
    ...
    override function create()
    {
        ...
        var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('gb/bg', 'shared'));
        ...
        gameJoltSheesh = new FlxSprite(-200, -100);
		gameJoltSheesh.frames = Paths.getSparrowAtlas('characters/GJ', 'shared');
		gameJoltSheesh.animation.addByPrefix('idle', 'gamejolt idle', 24);
		gameJoltSheesh.setGraphicSize(Std.int(gameJoltSheesh.width * 0.7));
		gameJoltSheesh.antialiasing = true;
        //gameJoltSheesh.flipX = true;
		add(gameJoltSheesh);
        ...
        gamejoltText.font = Paths.font('pixel.ttf');
        add(gamejoltText);

        tyBrrFrr = new FlxText(0, 25, 0, "Thanks BrightFyre for letting me use this code!!", 16);
        tyBrrFrr.screenCenter(X);
        tyBrrFrr.x -= 200;
        tyBrrFrr.color = FlxColor.fromRGB(84,155,149);
        tyBrrFrr.font = Paths.font('pixel.ttf');
        add(tyBrrFrr);
        ...
        if(GameJoltAPI.getStatus())
        {
            ...
            username.x += baseX;
            username.font = Paths.font('pixel.ttf');
            add(username);
        }
    }
    ...
    {
        super.beatHit();
        gameJoltSheesh.animation.play('idle', true);
    }
