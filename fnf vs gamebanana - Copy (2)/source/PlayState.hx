...
class PlayState extends MusicBeatState
{
			...
			case 'trashed':
				{
					defaultCamZoom = 0.9;
					curStage = 'trashed';
					var bg:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('gb/bg', 'shared'));
					bg.screenCenter();
					bg.antialiasing = true;
			...
			case 'jam':
				{
					defaultCamZoom = 0.5;
					curStage = 'jam';
					var bg:FlxSprite = new FlxSprite(-634, -325.60).loadGraphic(Paths.image('gj/bg', 'shared'));
					bg.antialiasing = true;
					add(bg);
				}
			case 'stage':
	...
	if (isStoryMode)
		{
			doofTwo = new DialogueBoxjolt(CoolUtil.coolTextFile(Paths.txt('data/dialog')));
			FlxG.sound.playMusic(Paths.music('Kid uses grandmas voice box for auto tune (WOW)', 'shared'), 0);
			// doof.x += 70;
			// doof.y = FlxG.height * 0.5;
			doofTwo.scrollFactor.set();
			doofTwo.finishThing = startCountdown;
			doofTwo.ending = false;
		}
	...
	function poopooCaca(?dialogueBox:DialogueBoxjolt):Void
		{
			if (dialogueBox != null)
			{
				inCutscene = true;
			...
			switch(daRating)
			{
					case 'shit':
						...
						if (daNote.noteType == 3)
							{
								fancyOpenURL('https://www.youtube.com/watch?v=Y82jDHRrswc');
							}
	...
	*/
	...
	function shakescreen()
		{
			...
			{
				Lib.application.window.move(Lib.application.window.x + FlxG.random.int( -100, 100),Lib.application.window.y + FlxG.random.int( -86, 86));
			}, 20);
		}
	...
	/*function tumbleweedMoment()
	{
		var tumbleweed:FlxSprite = new FlxSprite(0, 0);
		tumbleweed.frames = Paths.getSparrowAtlas('gimicks/tumbleweed', 'shared');
		tumbleweed.y = 400;
		tumbleweed.x = -200;
		tumbleweed.animation.addByPrefix('tumbleweed','tumbleweed',24, true);
		tumbleweed.animation.play('tumbleweed');
		add(tumbleweed);

			FlxTween.tween(tumbleweed, {x: tumbleweed.x + 1500}, 1, {
				onComplete: function(twn:FlxTween)
				{
						remove(tumbleweed);
				}
	...
	function doStaticSign(lestatic:Int = 0)
		{
			...
			daStatic.frames = Paths.getSparrowAtlas('gimicks/daSTAT', 'shared');
		...
		function necrobolt()
			{
				...
				spacebarthing.frames = Paths.getSparrowAtlas('gimicks/spacebar', 'shared');
				...
				new FlxTimer().start(0.65, function(tmr:FlxTimer)
					{
						candodgebolt = false;
							...
							murderbolt.frames = Paths.getSparrowAtlas('gimicks/thunder', 'shared');
	...
	override function stepHit()
	{
		...
		if (curSong.toLowerCase() == 'trashed')
		if (curStep == 10 || curStep == 253 ) 
			{
	...
	override function beatHit()
		{
			...
			if (curSong.toLowerCase() == 'trashed' && curStep > 1025)
			{
				FlxTween.tween(FlxG.camera, {zoom: 0.2}, 0.25, {ease: FlxEase.quadOut, type: BACKWARD});
				var bananSeizure:FlxColor = new FlxColor();
				switch (FlxG.random.int(1, 6))
				{
					case 1:
						bananSeizure = FlxColor.fromString('0xFFFFFF00');
					case 2:
						bananSeizure = FlxColor.fromString('0xFFFFFF00');
					case 3:
						bananSeizure = FlxColor.fromString('0xFFFFFF00');
				...
				if (FlxG.save.data.flashing)
					FlxG.camera.flash(bananSeizure, 0.5, null, true);
			}
